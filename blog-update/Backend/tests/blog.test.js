...
describe('root path API calls', () => {
  ...
  test('a new blog can be posted', async () => {
    ...
    expect(titles).toContain(
      'Testing posting to a bad blog website, an exercise in futility'
    )
  })
})
...
describe('deleting posts', () => {
  test('remove last blog in posts', async () => {
    ...
    await api.delete(`/api/blogs/${finalBlog._id}`).expect(204)
  })
})
...
describe('updating a post', () => {
  test('update first blogs author', async () => {
    ...
    await api.put(`/api/blogs/${firstBlogId}`).send(newTitle).expect(200)
